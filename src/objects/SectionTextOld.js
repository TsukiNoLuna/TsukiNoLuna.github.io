...
export class SectionText
{
    ...
    sample_coordinates()
    {
        ...
        let textPoints = [];
        //this.offset = new THREE.Vector3(-315, -50, 500);
        //this.offset = new THREE.Vector3(-wTexture/h, -hTexture/2, 500);
            ...
            for (let i = 0; i < this.textCanvas.height; i += samplingStep) {
                for (let j = 0; j < this.textCanvas.width; j += samplingStep) {
                    ...
                    if (imageData.data[(j + i * this.textCanvas.width) * 4] > 0) {
                        ...
                        textPoints.push(new THREE.Vector3(j + this.offset.x, i + this.offset.y, this.offset.z));
                    }
                }
            }
        }
        ...
        this.offset = new THREE.Vector3(wTexture/2, hTexture/2 + 400, 500);
        this.textCloud.position.copy(this.offset);
        //this.textCloud.lookAt(this.camera);
        //this.textCloud.rotateX(Math.PI/5);
    ...
    _onUpdate(delta)
    {
        ...
        w.normalize();
        if(w.angleTo(v) < Math.PI/3)
        {
        if(!this.onScreen)
        {
            for(let i = 0; i < this.textLen; i++)
            {
            this._startTween(i, this.main.abortController.signal);
            }
            this.onScreen = true;
        }
        }
        ...
        {
        this.onScreen = false;
        this.main.abortController.abort();
        this.main.abortController = new AbortController();
        for(let i = 0; i < this.textLen; i++)
            {
